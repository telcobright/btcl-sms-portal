...
import { Download, Server, KeyRound, Network, Database, Terminal, Lock, Boxes, Globe, Eye } from 'lucide-react';
...
function Secret({ value }: { value: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="sec-real"><code>{value}</code></span>
    </span>
  );
}
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 sm:p-8">
        <div ref={docRef} className="doc">
          ...
          <p className="lead">
            The BTCL communication platform runs on <strong>two groups of servers</strong> — the <strong>SBC group</strong>
            (voice / contact-center / PBX) and the <strong>SMS group</strong> (SMS gateway). Each server runs specific
            services (RouteSphere, Config Manager, FreeSWITCH, etc.) and they share a private WireGuard overlay network.
          </p>
          <ul>
            <li><strong>SBC group — 4 servers</strong> (<code>sbc1</code>–<code>sbc4</code>), reached via gateway <code>114.130.145.75</code> on per-server SSH ports; private LAN <code>192.168.24.0/24</code>.</li>
            <li><strong>SMS group — 2 servers</strong> (<code>dell-sms-master</code>, <code>dell-sms-slave</code>), reached via gateway <code>114.130.145.70</code>; private subnet <code>10.246.7.0/x</code>.</li>
          </ul>
          <p>Each server is reached by SSH on its group&apos;s gateway IP using the server&apos;s dedicated port.</p>
...
ssh -p 40001 telcobright@114.130.145.75</pre>
          ...
          <div className="cred">
            ...
            <p style={{ fontSize: '12px', color: '#7c2d12', margin: '6px 0 0' }}>Passwords are hidden — click <strong>•••••••• click to view</strong> to reveal each one.</p>
            <table>
              <tbody>
                ...
                <tr><td><strong>SSH password — SBC host (114.130.145.75)</strong></td><td><Secret value="!mN+xVY^nfYySFCY" /></td></tr>
                <tr><td><strong>sudo password (on the VMs)</strong></td><td><Secret value="Takay1#$ane%%" /></td></tr>
                <tr><td><strong>SIP/PBX server hippbx (114.130.145.82, port 22)</strong></td><td>user <code>telcobright</code> / password <Secret value="Takay1#$ane%%" /></td></tr>
                <tr><td><strong>MySQL (10.10.194.10:3306)</strong></td><td>user <code>tbuser</code> / password <Secret value="Takay1takaane$" /></td></tr>
              </tbody>
            </table>
          </div>
...
sudo tail -f /var/log/routesphere/routesphere.log   # live logs</pre>
          ...
          <h2><Server className="w-5 h-5" /> 4. SBC group — 4 servers (sbc1–sbc4)</h2>
          <p>Voice / contact-center / PBX servers, reached via gateway <code>114.130.145.75</code> on per-server SSH ports; private LAN <code>192.168.24.0/24</code>.</p>
          <table>
            <thead><tr><th>Server</th><th>SSH</th><th>Private IP</th><th>Services running</th></tr></thead>
            <tbody>
          ...
          <h2><Server className="w-5 h-5" /> 5. SMS group — 2 servers</h2>
          <p>SMS gateway servers, reached via gateway <code>114.130.145.70</code>; private subnet <code>10.246.7.0/x</code>.</p>
          <table>
            <thead><tr><th>Server</th><th>SSH</th><th>Private IP</th><th>Services running</th></tr></thead>
            <tbody>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr><td>Kafka cluster</td><td><code>10.10.199.20</code>, <code>10.10.198.20</code>, <code>10.10.197.20</code> : <code>9092</code></td><td>3 broker nodes serving both groups. Config-reload topic <code>config_event_loader_&lt;tenant&gt;</code>.</td></tr>
